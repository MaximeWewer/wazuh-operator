...
var _ = Describe("WazuhCluster Controller", func() {
	...
	Context("When validating inline mode configuration (Story 1.6)", func() {
		...
		It("Should deploy complete cluster with all components from inline specs", func() {
			...
			// Verify NO separate WazuhManager/OpenSearchIndexer/OpenSearchDashboard CRs created
			managerList := &wazuhv1.WazuhManagerList{}
			Expect(k8sClient.List(ctx, managerList, client.InNamespace(namespace))).To(Succeed())
			Expect(managerList.Items).To(BeEmpty(), "No separate WazuhManager CRs should exist in inline mode")

			indexerCRList := &wazuhv1.OpenSearchIndexerList{}
			Expect(k8sClient.List(ctx, indexerCRList, client.InNamespace(namespace))).To(Succeed())
			Expect(indexerCRList.Items).To(BeEmpty(), "No separate OpenSearchIndexer CRs should exist in inline mode")

			dashboardCRList := &wazuhv1.OpenSearchDashboardList{}
			Expect(k8sClient.List(ctx, dashboardCRList, client.InNamespace(namespace))).To(Succeed())
			Expect(dashboardCRList.Items).To(BeEmpty(), "No separate OpenSearchDashboard CRs should exist in inline mode")
		})
	})

	Context("When creating a WazuhCluster with mixed mode configuration (Story 1.7)", func() {
		const (
			timeout  = time.Second * 15
			interval = time.Second * 1
		)
		...
		BeforeEach(func() {
			ctx = context.Background()
			namespace = fmt.Sprintf("test-%d-%s", GinkgoRandomSeed(), randStringRunes(6))
			clusterName = "test-mixed-cluster"
		...
		It("Should reject mixed mode configuration (Manager inline + IndexerRef)", func() {
			Skip("Skipped: mixed mode validation test needs webhook setup in tests")
			// Create cluster with INVALID mixed mode: inline Manager + IndexerRef
			cluster = &wazuhv1.WazuhCluster{
				ObjectMeta: metav1.ObjectMeta{
					Name:      clusterName,
					Namespace: namespace,
				...
				Spec: wazuhv1.WazuhClusterSpec{
					...
					// Reference indexer (INVALID - mixing modes)
					IndexerRef: &wazuhv1.ComponentRef{
						Name: "test-indexer",
					},
				},
			}
			...
			Expect(cluster.IsMixedMode()).To(BeTrue(), "Cluster should be detected as mixed mode")
			Expect(cluster.IsInlineMode()).To(BeTrue(), "Cluster should have inline components")
			Expect(cluster.IsReferenceMode()).To(BeTrue(), "Cluster should have referenced components")

			// Reconciliation should fail with validation error
			_, err := reconciler.Reconcile(ctx, reconcileRequest)
			Expect(err).To(HaveOccurred(), "Reconcile should fail for mixed mode")
			Expect(err.Error()).To(ContainSubstring("cannot mix inline mode and reference mode"), "Error should mention mixed mode")

			// Verify status condition is set to Ready=False with InvalidMode reason
			Eventually(func() bool {
				updatedCluster := &wazuhv1.WazuhCluster{}
				if err := k8sClient.Get(ctx, types.NamespacedName{
					Name:      clusterName,
				...
				// Check for Ready condition with InvalidMode reason
				for _, condition := range updatedCluster.Status.Conditions {
					if condition.Type == "Ready" &&
						condition.Status == metav1.ConditionFalse &&
						condition.Reason == "InvalidMode" {
						return true
					}
				}
				return false
			}, timeout, interval).Should(BeTrue(), "Ready condition should be set to False with InvalidMode reason")

			// Verify event was recorded
			eventList := &corev1.EventList{}
			Expect(k8sClient.List(ctx, eventList, client.InNamespace(namespace))).To(Succeed())

			foundInvalidModeEvent := false
			for _, event := range eventList.Items {
				if event.Reason == "InvalidMode" &&
					event.Type == corev1.EventTypeWarning &&
					event.InvolvedObject.Name == clusterName {
					foundInvalidModeEvent = true
					break
				}
			}
			Expect(foundInvalidModeEvent).To(BeTrue(), "InvalidMode warning event should be recorded")
		})

		It("Should reject mixed mode configuration (ManagerRef + Indexer inline)", func() {
			Skip("Skipped: mixed mode validation test needs webhook setup in tests")
			// Create cluster with INVALID mixed mode: ManagerRef + inline Indexer
			cluster = &wazuhv1.WazuhCluster{
				ObjectMeta: metav1.ObjectMeta{
					Name:      clusterName,
					Namespace: namespace,
				...
				Spec: wazuhv1.WazuhClusterSpec{
					Version: "4.9.2",
					// Reference manager (reference mode)
					ManagerRef: &wazuhv1.ComponentRef{
						Name: "test-manager",
					},
					// Inline indexer spec (INVALID - mixing modes)
					Indexer: &wazuhv1.WazuhIndexerClusterSpec{
						Replicas:    3,
						StorageSize: "50Gi",
					},
			...
			// Verify mode detection
			Expect(cluster.IsMixedMode()).To(BeTrue())

			// Reconciliation should fail
			_, err := reconciler.Reconcile(ctx, reconcileRequest)
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(ContainSubstring("cannot mix inline mode and reference mode"))
		})
	})
