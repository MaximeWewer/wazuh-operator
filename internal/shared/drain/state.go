...
var ValidTransitions = map[v1.DrainPhase][]v1.DrainPhase{
	v1.DrainPhaseIdle: {
		v1.DrainPhasePending,
	},
	v1.DrainPhasePending: {
		v1.DrainPhaseDraining,
		v1.DrainPhaseIdle, // canceled
	},
	v1.DrainPhaseDraining: {
		v1.DrainPhaseVerifying,
		v1.DrainPhaseFailed,
	},
	v1.DrainPhaseVerifying: {
		v1.DrainPhaseComplete,
		v1.DrainPhaseFailed,
	},
	v1.DrainPhaseComplete: {
		v1.DrainPhaseIdle,
	...
	v1.DrainPhaseFailed: {
		v1.DrainPhaseRollingBack,
		v1.DrainPhaseIdle, // manual reset
	},
	v1.DrainPhaseRollingBack: {
		v1.DrainPhasePending, // retry
		v1.DrainPhaseFailed,  // max retries exceeded
		v1.DrainPhaseIdle,    // manual reset
	},
}

// IsValidTransition checks if a transition from one phase to another is valid
func IsValidTransition(from, to v1.DrainPhase) bool {
...
func TransitionTo(status *v1.ComponentDrainStatus, to v1.DrainPhase, message string) error {
	if status == nil {
	...
	case v1.DrainPhasePending:
		// Initialize for new drain
		status.StartTime = &now
		status.Progress = 0
	case v1.DrainPhaseComplete:
		status.Progress = 100
	}
...
func InitializeDrainStatus() *v1.ComponentDrainStatus {
	return &v1.ComponentDrainStatus{
		Phase: v1.DrainPhaseIdle,
	}
}

// StartDrain initializes a new drain operation
func StartDrain(status *v1.ComponentDrainStatus, targetPod string, previousReplicas, targetReplicas int32) error {
	if status.Phase != v1.DrainPhaseIdle {
		return fmt.Errorf("cannot start drain: current phase is %s, expected Idle", status.Phase)
	}

	now := metav1.NewTime(time.Now())
	status.Phase = v1.DrainPhasePending
...
func UpdateProgress(status *v1.ComponentDrainStatus, progress int32, message string) {
	if progress < 0 {
		progress = 0
	}
	if progress > 100 {
...
func UpdateShardCount(status *v1.ComponentDrainStatus, count int32) {
	status.ShardCount = &count
}

// UpdateQueueDepth updates the remaining queue depth (for manager drain)
func UpdateQueueDepth(status *v1.ComponentDrainStatus, depth int64) {
	status.QueueDepth = &depth
}

// MarkFailed marks the drain operation as failed
func MarkFailed(status *v1.ComponentDrainStatus, reason string) error {
	return TransitionTo(status, v1.DrainPhaseFailed, reason)
}

// MarkComplete marks the drain operation as complete
func MarkComplete(status *v1.ComponentDrainStatus) error {
	return TransitionTo(status, v1.DrainPhaseComplete, "Drain completed successfully")
}

// StartRollback initiates a rollback after failure
func StartRollback(status *v1.ComponentDrainStatus, reason string) error {
	return TransitionTo(status, v1.DrainPhaseRollingBack, reason)
}

// ScheduleRetry schedules a retry attempt after rollback
func ScheduleRetry(status *v1.ComponentDrainStatus, retryTime time.Time) {
...
func IsRetryDue(status *v1.ComponentDrainStatus) bool {
...
func CanRetry(status *v1.ComponentDrainStatus, maxAttempts int32) bool {
	return status.AttemptCount < maxAttempts
}

// ResetForRetry resets status for a retry attempt
func ResetForRetry(status *v1.ComponentDrainStatus) error {
	if status.Phase != v1.DrainPhaseRollingBack {
...
func IsDrainInProgress(status *v1.ComponentDrainStatus) bool {
	if status == nil {
		return false
	}
	switch status.Phase {
...
func IsDrainFailed(status *v1.ComponentDrainStatus) bool {
	if status == nil {
		return false
	}
	return status.Phase == v1.DrainPhaseFailed
}

// IsDrainComplete returns true if the drain has completed successfully
func IsDrainComplete(status *v1.ComponentDrainStatus) bool {
	if status == nil {
		return false
	}
	return status.Phase == v1.DrainPhaseComplete
}

// GetDrainDuration returns the elapsed time since drain started
func GetDrainDuration(status *v1.ComponentDrainStatus) time.Duration {
...
func IsTimeout(status *v1.ComponentDrainStatus, timeout time.Duration) bool {
	if status == nil || status.StartTime == nil {
		return false
	}
...
func Reset(status *v1.ComponentDrainStatus) {
	if status == nil {
		return
	}
	now := metav1.NewTime(time.Now())
	status.Phase = v1.DrainPhaseIdle
	status.LastTransitionTime = &now
	status.TargetPod = ""
	status.PreviousReplicas = nil
	status.TargetReplicas = nil
