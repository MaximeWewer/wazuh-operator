...
func (r *ComponentTemplateReconciler) Delete(ctx context.Context, template *wazuhv1.OpenSearchComponentTemplate) error {
	...
	for _, ref := range template.Spec.ClusterRefs {
		...
		if err != nil {
			if errors.IsNotFound(err) {
				// The cluster itself is gone, so the component template went with it: nothing to delete.
		...
		if err := templatesAPI.DeleteComponentTemplate(ctx, template.Name); err != nil {
			...
			cleanupErrs = append(cleanupErrs, fmt.Sprintf("%s/%s: delete component template: %v", ref.Namespace, ref.Name, err))
			continue
		}
	...
	if len(cleanupErrs) > 0 {
		// Keep the finalizer and retry: the component template must actually be removed from
		// OpenSearch, not silently leaked when the API is unavailable or the delete fails.
		return fmt.Errorf("component template %q cleanup incomplete, will retry: %s", template.Name, strings.Join(cleanupErrs, "; "))
	}
