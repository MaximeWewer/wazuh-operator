...
func (b *ISMPolicyBuilder) Build() map[string]any {
	...
	if len(b.states) > 0 {
		states := make([]map[string]any, len(b.states))
		for i, state := range b.states {
			...
			if len(state.Actions) > 0 {
				actions := make([]map[string]any, len(state.Actions))
			...
			if len(state.Transitions) > 0 {
				transitions := make([]map[string]any, len(state.Transitions))
				for j, transition := range state.Transitions {
					transitions[j] = map[string]any{
	...
	if len(b.ismTemplate) > 0 {
		templates := make([]map[string]any, len(b.ismTemplate))
...
func DefaultWazuhISMPolicy() map[string]any {
	return NewISMPolicyBuilder(constants.ISMPolicyName).
		...
		AddState(ISMState{
			...
			Actions: []ISMAction{
				{Type: "rollover", Config: map[string]any{
					"min_index_age": constants.ISMHotStateMinIndexAge,
					"min_doc_count": constants.ISMHotStateMinDocCount,
				}},
			},
			Transitions: []ISMTransition{
				{StateName: constants.ISMStateWarm, Conditions: map[string]any{
		...
		AddState(ISMState{
			...
			Actions: []ISMAction{
				{Type: "replica_count", Config: map[string]any{
					"number_of_replicas": constants.ISMWarmStateReplicas,
				}},
			},
			Transitions: []ISMTransition{
				{StateName: constants.ISMStateDelete, Conditions: map[string]any{
		...
		AddState(ISMState{
			...
			Actions: []ISMAction{
				{Type: "delete", Config: map[string]any{}},
			},
		}).
