...
func (b *RoleBuilder) Build() map[string]any {
	role := map[string]any{
	...
	if len(b.indexPermissions) > 0 {
		indexPerms := make([]map[string]any, len(b.indexPermissions))
		for i, perm := range b.indexPermissions {
			indexPerms[i] = map[string]any{
	...
	if len(b.tenantPermissions) > 0 {
		tenantPerms := make([]map[string]any, len(b.tenantPermissions))
		for i, perm := range b.tenantPermissions {
			tenantPerms[i] = map[string]any{
...
func DefaultWazuhRoles() map[string]map[string]any {
