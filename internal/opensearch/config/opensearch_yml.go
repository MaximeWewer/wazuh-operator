...
func (c *OpenSearchConfig) Build() string {
	...
	if len(c.NodeAttributes) > 0 {
		...
		for key, value := range c.NodeAttributes {
			fmt.Fprintf(&sb, "node.attr.%s: %s\n", key, value)
		}
	}
	sb.WriteString("\n")

	// Path settings
	fmt.Fprintf(&sb, "path.data: %s\n", c.PathData)
	fmt.Fprintf(&sb, "path.logs: %s\n\n", c.PathLogs)

	// Network settings
	fmt.Fprintf(&sb, "network.host: %s\n", c.NetworkHost)
	fmt.Fprintf(&sb, "http.port: %d\n", c.HTTPPort)
	fmt.Fprintf(&sb, "transport.port: %d\n\n", c.TransportPort)
	...
	if c.SecurityEnabled {
		...
		fmt.Fprintf(&sb, "plugins.security.ssl.transport.pemcert_filepath: %s/tls.crt\n", c.CertPath)
		fmt.Fprintf(&sb, "plugins.security.ssl.transport.pemkey_filepath: %s/tls.key\n", c.CertPath)
		fmt.Fprintf(&sb, "plugins.security.ssl.transport.pemtrustedcas_filepath: %s/ca.crt\n\n", c.CertPath)

		fmt.Fprintf(&sb, "plugins.security.ssl.http.pemcert_filepath: %s/tls.crt\n", c.CertPath)
		fmt.Fprintf(&sb, "plugins.security.ssl.http.pemkey_filepath: %s/tls.key\n", c.CertPath)
		fmt.Fprintf(&sb, "plugins.security.ssl.http.pemtrustedcas_filepath: %s/ca.crt\n\n", c.CertPath)
