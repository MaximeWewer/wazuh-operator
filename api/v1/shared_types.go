...
// GatewayAPISpec defines Gateway API configuration for exposing services
// via HTTPRoute, TCPRoute, and UDPRoute instead of legacy Ingress
type GatewayAPISpec struct {
	// Enable Gateway API instead of Ingress
	// +optional
	Enabled bool `json:"enabled,omitempty"`
	...
	// Hostnames for HTTPRoute
	// +optional
	Hostnames []string `json:"hostnames,omitempty"`

	// HTTP route configuration (Dashboard, Manager API)
	// +optional
	HTTP *GatewayHTTPConfig `json:"http,omitempty"`

	// TCP route configuration (Agent enrollment/events)
	// +optional
	TCP *GatewayTCPConfig `json:"tcp,omitempty"`

	// UDP route configuration (Syslog)
	// +optional
	UDP *GatewayUDPConfig `json:"udp,omitempty"`
}

// GatewayReference defines a reference to a Gateway resource
type GatewayReference struct {
	// Name of the Gateway
	// +kubebuilder:validation:Required
	Name string `json:"name"`

	// Namespace of the Gateway (defaults to same namespace as the resource)
	// +optional
	Namespace string `json:"namespace,omitempty"`

	// SectionName is the name of a section within the Gateway
	// +optional
	SectionName string `json:"sectionName,omitempty"`
}

// GatewayHTTPConfig defines HTTP route configuration
type GatewayHTTPConfig struct {
	// Annotations for the HTTPRoute resource
	// +optional
	Annotations map[string]string `json:"annotations,omitempty"`

	// PathPrefix for the HTTP route (default: "/")
	// +optional
	// +kubebuilder:default="/"
	PathPrefix string `json:"pathPrefix,omitempty"`
}

// GatewayTCPConfig defines TCP route configuration for agent traffic
type GatewayTCPConfig struct {
	// Enable TCP routes for agent traffic
	// +optional
	Enabled bool `json:"enabled,omitempty"`

	// EnrollmentEnabled enables TCP route for agent enrollment (port 1515)
	// +optional
	EnrollmentEnabled bool `json:"enrollmentEnabled,omitempty"`

	// EventsEnabled enables TCP route for agent events (port 1514)
	// +optional
	EventsEnabled bool `json:"eventsEnabled,omitempty"`

	// ClusterEnabled enables TCP route for cluster communication (port 1516)
	// +optional
	ClusterEnabled bool `json:"clusterEnabled,omitempty"`

	// Annotations for the TCPRoute resources
	// +optional
	Annotations map[string]string `json:"annotations,omitempty"`
}

// GatewayUDPConfig defines UDP route configuration for syslog
type GatewayUDPConfig struct {
	// Enable UDP route for syslog
	// +optional
	Enabled bool `json:"enabled,omitempty"`

	// SyslogPort is the UDP port for syslog (default: 514)
	// +optional
	...
	// Annotations for the UDPRoute resource
	// +optional
	Annotations map[string]string `json:"annotations,omitempty"`
}
...
type AntiAffinitySpec struct {
	...
	// Common values: kubernetes.io/hostname (default), topology.kubernetes.io/zone
	// +optional
	// +kubebuilder:default="kubernetes.io/hostname"
	TopologyKey string `json:"topologyKey,omitempty"`

	// Type specifies whether the anti-affinity rule is required (hard) or preferred (soft)
	// "required": Pods MUST be scheduled on different topology domains (pods stay Pending if not possible)
	...
	// Higher weight means the scheduler tries harder to satisfy this preference
	// Only applicable when Type=preferred, ignored for Type=required
	// +optional
	// +kubebuilder:validation:Minimum=1
...
// Supports both native Kubernetes Secrets and External Secrets Operator (ESO)
type CredentialsSecretRef struct {
	// SecretName references a native Kubernetes Secret
	// +optional
	SecretName string `json:"secretName,omitempty"`

	// ExternalSecretRef references an ExternalSecret from ESO
	// When specified, the operator will read credentials from the K8s Secret
	// created by the ExternalSecret controller
	// +optional
...
// GetSecretName returns the name of the K8s Secret to read credentials from
func (c *CredentialsSecretRef) GetSecretName() string {
	if c == nil {
		return ""
	}
	if c.ExternalSecretRef != nil {
		return c.ExternalSecretRef.Name
	}
	return c.SecretName
}

// IsExternalSecret returns true if credentials come from an ExternalSecret
func (c *CredentialsSecretRef) IsExternalSecret() bool {
	return c != nil && c.ExternalSecretRef != nil
}
...
type HPASpec struct {
	// Enabled enables HPA for the component
	// +optional
	// +kubebuilder:default=false
	Enabled bool `json:"enabled,omitempty"`
	...
	// Behavior configures the scaling behavior (optional advanced config)
	// +optional
	// +kubebuilder:pruning:PreserveUnknownFields
	Behavior *HPABehavior `json:"behavior,omitempty"`
}

// HPABehavior configures scaling behavior for HPA
...
// HPAScalingRules configures scaling rules
type HPAScalingRules struct {
	// StabilizationWindowSeconds is the stabilization window in seconds
	// +optional
	// +kubebuilder:validation:Minimum=0
	// +kubebuilder:validation:Maximum=3600
	StabilizationWindowSeconds *int32 `json:"stabilizationWindowSeconds,omitempty"`

	// SelectPolicy is the policy selection (Max, Min, Disabled)
	// +optional
