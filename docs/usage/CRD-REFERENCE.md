...
- **Automatic in-cluster migration (forward).** On first introduction of a split path, a `migrate-data` init container copies the existing data from `wazuh-data` into the new PVC once (idempotent via a `.migrated` marker). The source is **never deleted** (implicit backup); the `wazuh-data` volume does not shrink.
- **Automatic in-cluster migration (reverse).** When a path is **removed** from `volumeClaims`, during the recreation window (StatefulSet absent, PVCs detached) the operator runs a one-shot Job that copies the dedicated PVC's current data **back into `wazuh-data`** at its original subPath, then **deletes** the now-orphaned PVC. No data is stranded. If the Job fails, the PVC is kept for manual recovery (a `ReverseMigrationFailed` event is emitted).
- **Recreation on layout change.** Adding/removing a path, or changing a `storageClass`/`accessMode`, recreates the manager StatefulSet (VolumeClaimTemplates are immutable). The `wazuh-data` PVC (holding `client.keys`) is always **preserved** and re-adopted. This causes **downtime** for the master (single replica); workers roll per replica.
- **Per-path expansion.** Increasing a `size` (the default `storageSize` or any per-path `size`) is applied **in place** (no recreation): the operator resizes each PVC to its own declared size, provided its StorageClass has `allowVolumeExpansion: true`. Shrinking is rejected by Kubernetes.
