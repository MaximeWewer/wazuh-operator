...
Returns the spec with sizing profile applied to resources, replicas, and storage
...
{{- else if eq $profile "S" -}}
  {{- /* S: Small profile for dev/test environments */ -}}
  {{- $profileIndexer := dict "replicas" 1 "storageSize" "20Gi" "javaOpts" "-Xms1g -Xmx1g -Dlog4j2.formatMsgNoLookups=true" "resources" (dict "requests" (dict "cpu" "500m" "memory" "1Gi") "limits" (dict "cpu" "1" "memory" "2Gi")) -}}
...
{{- else if eq $profile "M" -}}
  {{- /* M: Medium profile for small production environments */ -}}
  {{- $profileIndexer := dict "replicas" 3 "storageSize" "50Gi" "javaOpts" "-Xms2g -Xmx2g -Dlog4j2.formatMsgNoLookups=true" "resources" (dict "requests" (dict "cpu" "2" "memory" "4Gi") "limits" (dict "cpu" "4" "memory" "8Gi")) -}}
...
{{- else if eq $profile "L" -}}
  {{- /* L: Large profile for production environments */ -}}
  {{- $profileIndexer := dict "replicas" 3 "storageSize" "100Gi" "javaOpts" "-Xms4g -Xmx4g -Dlog4j2.formatMsgNoLookups=true" "resources" (dict "requests" (dict "cpu" "4" "memory" "8Gi") "limits" (dict "cpu" "8" "memory" "16Gi")) -}}
...
{{- else if eq $profile "XL" -}}
  {{- /* XL: Extra large profile for enterprise environments */ -}}
  {{- $profileIndexer := dict "replicas" 5 "storageSize" "200Gi" "javaOpts" "-Xms8g -Xmx8g -Dlog4j2.formatMsgNoLookups=true" "resources" (dict "requests" (dict "cpu" "8" "memory" "16Gi") "limits" (dict "cpu" "16" "memory" "32Gi")) -}}
